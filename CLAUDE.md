...
- **Browse/search** (`/posts`, auth-guarded, `src/pages/PostsBrowse`): a navbar search bar (`src/components/SearchBar`, signed-in users only, sits between "New post" and the avatar) parses `#tag word word` into tag filters and title words and navigates to `/posts?q=...`; the page lists matching posts (Most recent / Top score sort, Load more pagination) via the `search_posts()` Postgres function (migration 14), which requires a post to match every given word and carry every given tag.
